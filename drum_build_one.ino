...
void setValues(){
  for (int beat =  0; beat < 16; beat++) {
    if (instrumentSelect == 0) {
      ...
      if (polySeed < 1){
        stepValuesPoly[beat] = cowbellFiles[random(4, 13)]; //should give us just non-'null' notes. may be a more elegant way to do off notes in future!
      } else {
        stepValuesPoly[beat] = 0;
      }
      if (switchButtonCounter > 3) { //always have poly in random mode (but not always right at the start)
        stepValuesPoly[0] = cowbellFiles[random(4,13)];
      }
      instPlayingNow = 1;
    }
  }
    ...
    for (int beat = 0; beat <16; beat++) {
      ...
      if (shuffSeed < 1) {
        if (instPlayingNow == 0){
          ...
          if (polySeed < 1) {
            stepValuesPoly[beat] = cowbellFiles[random(4, 13)];
          }
